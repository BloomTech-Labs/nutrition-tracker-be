const actual_fat_macros_over_time = (user_id, start_date, end_date) => {
  ...
  return `
    select 
      ${user_id} as user_id,
      od.observation_date,
      COALESCE(ROUND(d_n_t.fat_calories / 9, 2), 0) as actual_fat_macros
    from ${observation_dates} as od
    left join (select date, fat_calories from daily_nutrition_totals where user_id = ${user_id}) as d_n_t
    on 
